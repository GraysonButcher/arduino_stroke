...
void evaluateInputStates()
{
  ...
  if ((forceVal < pullThresholdlow) && (pullInitiated == 1)) {
    ...
    if ((responseDuration >= holdCriterion) && (holdMetThisPull == 1)) {
      thisHold = 2;
      //      Serial.print("this hold is: ");
      //      Serial.println(thisHold);
...
void evaluateShapingStep() {
  ...
  if ((cumulativeAttempts >= 5) && (attemptsThisSession >= 2) && (holdArrayAverage == 1)) { //should be able to switch this conditionals to JUST being that the 1.8 is met. 
    Serial.println("threshold met, moving on");
    activationThreshold = activationThreshold + 200; //need a more exact computation or multiple lines to represent the actual shaping steps I want now.
    cumulativeAttempts = 0; //I think I can comment cumulativeAttempts and attemptsthissession out and replace with resetting the array to all zeros.
